...
clim <- read.csv("C:/School/WUR/Thesis/JoAB/Corrected Analyses/climate_data_13_23.csv",)
...
baseline <- lmer(fledge ~ maleplumage + cor_Hatching_day_c + 
...
relative_nrFledge <- slidingwin(baseline = 
                                  lmer(fledge ~ maleplumage + cor_Hatching_day  +
                                         (1|ring_female), data = bio, REML = F),
